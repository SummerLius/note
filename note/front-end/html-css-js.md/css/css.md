...
3. [外边距属性：margin-top、margin-right、margin-bottom、margin-left和margin](#)
    1. [折叠外边距](#)
        ...
        - **判断两个margin是相邻（adjoining）的，当且仅当满足一下三种情况**：
            ...
            - 都属于 垂直-相邻盒边界，（vertically-adjacent box edges），即形成下列某一对：
...
5. **浮动 Floats**
    ...
    2. 浮动定位：float属性
        - 适用元素：除了绝对定位的absolutely positioned元素（待确定）
        - 值选项：
            - none：默认值，不浮动
            - left：
            - right：
        - **以下是控制浮动行为的准确规则**：
            1. 左浮动盒的左外边界不能位于其包含块的左边界的左边。向右浮动的元素也有类似规则。
                - > 自己分析：就是浮动的元素的位置，还是父元素内部，而不管其父元素是否是浮动
    ...
    3. 控制float旁边的流：clear属性
        ...
        - 值选项：
            ...
            - both：
        - 说明：
            - 值不为none，就隐含了要引入空隙（clearance）。
            - 空隙会阻止外边距合并，并作为元素margin-top之上的空间，用来在垂直方向上把元素向下推，从而给浮动元素腾出空间。
            - 想要计算设置了`clear`属性的元素的空隙，先要确定该元素的上边框边界的假定位置，该位置是当元素clear属性设置为none时，其上边框边界实际所在的位置。
            - **如果该元素的上边框的假定位置没有越过相关的浮动元素，那么就得引入空隙，并根据8.3.1中的规则合并外边距；如果该元素margin-top设置的足够大以至于越过了浮动元素，或该元素上方根本没有浮动元素，那么就不用引入空隙，即此时该元素设置的clear属性不会起作用**。
            - 空隙高度计算：
                1. 然后，空隙的高度被设置为下面两者的最大值：
                    - 把块的边框边界放在最低的将被clear的浮动元素的下外边界，所必需的高度
                    - 把块的上边框边界放在其假定位置（clear为none时的位置），所需要的高度
                2. **或者，空隙被精确的设置为把块的border边，放在最低的将被clear的浮动元素的下外边界，所需要的高度**
                - 注意：两种行为都允许用现有的web内容评估其兼容性，将来的CSS规范将规定其一
            - > 注意：空隙可以为负数或0
        - > 关于空隙clearance，可以查看该链接：https://www.w3cplus.com/css/understanding-bfc-and-margin-collapse.html
