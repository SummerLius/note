    ...
    - W3C事件流模型：
        ...
        - API：
            ...
            - `EventTarget.prototype.dispatchEvent(event)`
                - 派遣事件，一般该方法有浏览器自行调用，如果开发者调用，那么该事件event也会经历完整的事件流，即`捕获 -> target -> 冒泡`，并且该事件event的target属性为该方法的调用对象，例如`document.getElementById("x").dispatchEvent(event)`，那么其event.target为document.getElementById("x")元素对象
    - 冒泡模型兼容性好，默认用该模型即可
    - 事件代理/委托
